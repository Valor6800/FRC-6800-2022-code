...
Leds::Leds(frc::TimedRobot *_robot, Elevarm *_elevarm, Intake *_intake, Drivetrain *_drivetrain) : BaseSubsystem(_robot, "Leds"),
    elevarm(_elevarm),
    intake(_intake),
...
void Leds::resetState(){
    ...
    // candle.setColor(CANdleSensor::toRGB(0xEEA800));
}
...
void Leds::analyzeDashboard(){
    ...
    if (elevarm->futureState.pitModeEnabled) {
        candle.setColor(CANdleSensor::RGBColor(255, 90, 0));
    } else if (robot->IsDisabled()) {
        if (elevarm->futureState.wristInRange) candle.setColor(1, CANdleSensor::RGBColor(0, 255, 0));
        else candle.setColor(1, CANdleSensor::RGBColor(255, 0, 0));
        if (elevarm->futureState.armInRange) candle.setColor(2, CANdleSensor::RGBColor(0, 255, 0));
        else candle.setColor(2, CANdleSensor::RGBColor(255, 0, 0));
    } else if (robot->IsAutonomous()) {
        if (elevarm->futureState.atCarriage && elevarm->futureState.atArm && elevarm->futureState.atWrist){
            candle.setColor(CANdleSensor::RGBColor(0, 255, 0));
        } else{
            candle.setColor(CANdleSensor::RGBColor(0, 0, 255));
        }
    } else if (robot->IsEnabled()){
        if (intake->getFuturePiece() == Piece::CUBE) {
            candle.setColor(1, CANdleSensor::RGBColor(156, 0, 255));
        } else {
            candle.setColor(1, CANdleSensor::RGBColor(255, 196, 0));
        }

        if (intake->state.intakeState == Intake::IntakeStates::SPIKED){
            candle.setColor(2,CANdleSensor::RGBColor(255, 0, 0));
        } else {
            candle.setColor(2, CANdleSensor::RGBColor(255, 255, 255));
        }
    }
